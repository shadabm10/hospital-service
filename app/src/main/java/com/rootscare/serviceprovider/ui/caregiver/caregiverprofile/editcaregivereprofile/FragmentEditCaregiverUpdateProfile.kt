            ...
            if (getDoctorProfileResponse?.result != null) {
                with(fragmentDoctorEditProfileBinding!!) {
                    ...
                    if (getDoctorProfileResponse.result.department != null && getDoctorProfileResponse.result.department.equals("")) {
                        textViewDepartment.setText("No Department")
                        departTitle = ""
                        departmentId = "1"
        ...
        with(fragmentDoctorEditProfileBinding!!) {
            if (fragmentEditDoctorProfileViewModel?.appSharedPref?.loginUserId != null && checkValidationForRegStepOne()) {
                ...
                if (certificateListAdapter?.qualificationDataList != null && certificateListAdapter?.qualificationDataList!!.size > 0) {
                    val tempList: ArrayList<com.rootscare.data.model.api.response.caregiver.profileresponse.QualificationDataItem> = ArrayList()
...
//            activityLoginBinding?.edtPassword?.setError("Please enter Password")
    ...
    private fun addCertificatePortionsSetUp() {
        with(fragmentDoctorEditProfileBinding!!) {
            ...
            var fragment = CertificateUploadFragmentCaregiver.newInstance()
            fragment.listener = object : CertificateUploadFragmentCaregiver.PassDataCallBack {
                override fun onPassData(data: com.rootscare.data.model.api.response.caregiver.profileresponse.QualificationDataItem) {
