...
class FragmentHome: BaseFragment<FragmentHomeBinding, FragmentHomeVirewModel>(), FragmentHomeNavigator {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        fragmentHomeBinding?.cardViewManageProfile?.setOnClickListener(View.OnClickListener {
            (activity as HomeActivity).checkFragmentInBackstackAndOpen(
                FragmentDoctorProfile.newInstance())
        })

        fragmentHomeBinding?.cardViewReviewAndRating?.setOnClickListener(View.OnClickListener {
            (activity as HomeActivity).checkFragmentInBackstackAndOpen(
                FragmentReviewAndRating.newInstance())
        })

        fragmentHomeBinding?.cardViewMyAppointment?.setOnClickListener(View.OnClickListener {
            (activity as HomeActivity).checkFragmentInBackstackAndOpen(
                FragmentMyAppointment.newInstance())
        })

        fragmentHomeBinding?.cardViewMySchedule?.setOnClickListener(View.OnClickListener {
            (activity as HomeActivity).checkFragmentInBackstackAndOpen(
